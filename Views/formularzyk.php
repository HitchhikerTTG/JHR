...
document.addEventListener('DOMContentLoaded', function() {
    const checkboxes = document.querySelectorAll('input[name="feature_list[]"]');
    const submitBtn = document.querySelector('button[type="submit"]');
    const komunikatDiv = document.getElementById('info');
    const form = document.querySelector('form');
    
    function updateSubmitButton() {
        const checkedCount = document.querySelectorAll('input[name="feature_list[]"]:checked').length;
        
        if (checkedCount === 8) {
            submitBtn.disabled = false;
            komunikatDiv.textContent = 'Wybrano 8 cech - możesz utworzyć okno';
            komunikatDiv.style.color = 'green';
        } else if (checkedCount < 8) {
            submitBtn.disabled = true;
            komunikatDiv.textContent = `Wybierz jeszcze ${8 - checkedCount} cech`;
            komunikatDiv.style.color = 'orange';
        } else {
            submitBtn.disabled = true;
            komunikatDiv.textContent = `Wybrano za dużo cech (${checkedCount}). Usuń ${checkedCount - 8} cech`;
            komunikatDiv.style.color = 'red';
        }
    }
    
    // Dodaj event listenery do wszystkich checkboxów
    checkboxes.forEach(function(checkbox) {
        checkbox.addEventListener('change', updateSubmitButton);
    });
    
    // Zapobiegaj wysłaniu formularza jeśli nie wybrano 8 cech
    form.addEventListener('submit', function(e) {
        const checkedCount = document.querySelectorAll('input[name="feature_list[]"]:checked').length;
        if (checkedCount !== 8) {
            e.preventDefault();
            alert('Musisz wybrać dokładnie 8 cech przed utworzeniem okna!');
            return false;
        }
    });
    
    // Inicjalna aktualizacja stanu przycisku
    updateSubmitButton();
});
</script>