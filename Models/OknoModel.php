...
class OknoModel extends Model
{
    ...
    protected $allowedFields = ['hash', 'wlasciciel', 'nazwa', 'kluczyk', 'imie_wlasciciela', 'id_zestaw_cech'];
    ...
    public function analizyCechOkna($hashOkna, $hashWlasciciela, $zestawCech = null) {
        $przypisaneCechyModel = model(\App\Models\PrzypisaneCechyModel::class);

        // Pobierz cechy dla odpowiedniego zestawu
        $cechyModel = model(\App\Models\CechyModel::class);

        // Jeśli nie przekazano zestawu cech, sprawdź z okna
        if ($zestawCech === null) {
            $zestawCech = $this->getWindowFeatureSet($hashOkna, $hashWlasciciela);
        }

        $wszystkieCechy = $cechyModel->listFeatures($zestawCech);
        $wszystkieZapisaneCechy = $przypisaneCechyModel->cechyOkna($hashOkna);
        $nazwaneCechy = $cechyModel->listFeatures(); // To jest nadal potrzebne do konwersji ID na nazwy
        ...
        foreach ($wszystkieZapisaneCechy as $zapisanaCecha) {
            ...
            // Zliczamy ogólną częstotliwość tylko jeśli cecha istnieje w danym zestawie
            if (in_array($cechaId, array_column($wszystkieCechy, 'id'))) {
        ...
        // Określamy cechy, które są dostępne w danym zestawie
        $dostepneCechyWZestawie = array_column($wszystkieCechy, 'id');
        $pozostale = array_diff($dostepneCechyWZestawie, $arena ?? [], $wskazane, $prywatne);
    ...
    private function konwertujCechyNaNazwy($cechy, $nazwaneCechy)
    {
        ...
        foreach ($cechy as $cechaID) {
            // Znajdź nazwę cechy w tablicy $nazwaneCechy na podstawie ID
            $nazwaCechy = '';
            foreach ($nazwaneCechy as $cecha) {
                if ($cecha['id'] == $cechaID) {
                    $nazwaCechy = $cecha['cecha_pl'];
                    break;
                }
            }
            if (!empty($nazwaCechy)) {
                 $wynik[] = [$cechaID, $nazwaCechy];
            }
        }
